# ChatGPT Images 2.0 - Hook Only Prompt

Use this only for slide 1, the custom hook image.

Production rule: do not generate all slideshow images with Images 2.0. Every slideshow must use exactly one Images 2.0 image for slide 1, then use approved Supabase visual-library assets for the remaining slides with local text overlays.

```text
Create one photorealistic vertical 9:16 hook image for a running coaching slideshow.

Slide role:
Slide 1 hook image only.

Subject:
<topic-specific scene>

Selected visual world:
<one route/world only: forest road, city park path, track edge, quiet neighborhood road, coastal path, or treadmill / indoor gym>
...
- Pre-workout: preparing before the session starts, light warmup movement, calm anticipation, route visible.
- During-workout: steady controlled running effort, natural stride, consistent cadence, no stutter step.
- Post-workout: cooldown or just-finished moment, sweaty, satisfied, grounded, breathing settling.
...
Composition:
Tight medium close-up, chest-to-waist framing, runner feels close to the camera but still in a real outdoor route.
Create strong negative space for large overlay hook text.
Do not bake any text into the image.
...
No app UI.
No watermarks.
No static hands-on-hips hero pose.
No mixed route worlds.
No distorted limbs, extra fingers, Apple Watch, Garmin watch, smartwatch, GPS watch, fake watch close-ups, readable watch UI, fake steam, exaggerated model smile, bland clean park-portrait look, or unnatural head turns.

Output:
One vertical image only.
```
