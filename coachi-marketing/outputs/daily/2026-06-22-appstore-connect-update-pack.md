# Coachi App Store Connect Update Pack - 2026-06-22
...
The live App Store audit scored the current listing `43/100` (`conversion_risk`). The biggest conversion gaps are:

- Title casing reads unfinished: `Coachi: Voice fitness Coach`.
- Title and subtitle do not clearly claim running.
- The first copy is broader than the runner-led traffic promise.
- US listing has `0` visible ratings.
...
1. `Live AI run coaching`
   ...
   Visual: runner in motion with iPhone and Apple Watch visible; show active workout voice cue, not setup.
...
4. `Build the run in seconds`
   ...
   Visual: workout setup screen, but after live coaching and HR value have been established.
...
6. `Share the result`
   ...
   Visual: share card or workout result, cleaner and less stock-like than the current group-running frame.
